        ...
        for dict in data:
            ...
            for item_data in items:
                ...
                if item_name == '':
                    ...
                    if item_amount == '' or item_amount == 'null' or item_amount == None:
...
@login_required
def create_lists(request, store_id): #list
    ...
    if request.method == 'POST':
        ...
        for dict in data:
            ...
            try:
                if list_type == '':
                    list = List(name=name, store=store)
                else:
                    list = List(name=name, type=list_type, store=store)
                list.full_clean()
                list.save()
            except ValidationError as e:
                # print(e.messages)
...
def create_list_item(request, list_id, item_id): #list_item
    ...
    if request.method == 'POST':
        ...
        return JsonResponse({"message": "Update successful.", "created":created}, status=201)
        # return list_item..
